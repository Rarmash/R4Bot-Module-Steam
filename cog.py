...
class Steam(commands.Cog):
    ...
    def get_steam_id(self, ctx, steamid64, url_or_username=None):
        if steamid64:
            return steamid64
        if url_or_username:
            return None
    ...
    def steam_api_get(self, endpoint, **params):
    ...
    def resolve_vanity_via_community(self, vanity: str):
        response = requests.get(
    ...
    def resolve_steam_id(self, steamid64=None, url_or_username=None):
        direct_steam_id, vanity = self.parse_steam_identity(steamid64=steamid64, url_or_username=url_or_username)
        if direct_steam_id:
        ...
        try:
            payload = self.steam_api_get('ISteamUser/ResolveVanityURL/v0001/', vanityurl=vanity)
        ...
        return self.resolve_vanity_via_community(vanity)
    ...
    def get_player_level(self, steam_id):
        payload = self.steam_api_get('IPlayerService/GetSteamLevel/v1/', steamid=steam_id)
    ...
    @classmethod
    def extract_creation_timestamp_from_text(cls, text: str):
        patterns = [
        ...
        for pattern in patterns:
            ...
            timestamp = cls.parse_english_date_to_timestamp(match.group(1).strip())
            if timestamp:
                return timestamp

        return None

    def get_profile_creation_timestamp(self, summary, steam_id):
        time_created = summary.get('timecreated')
        if time_created:
            return time_created

        profile_url = self.normalize_profile_url(summary, steam_id)
        if not profile_url:
            return None
        ...
        timestamp = self.extract_creation_timestamp_from_text(xml_response.text)
        if timestamp:
            return timestamp
        ...
        return self.extract_creation_timestamp_from_text(html_response.text)
        ...
        if isinstance(error, requests.HTTPError) and error.response is not None:
            if error.response.status_code == 403:
                return (
                    ...
                    'Сейчас в config/secrets/steam.json нужен настоящий Steam Web API key.'
                )
    ...
    async def profile(self, ctx: discord.ApplicationContext, steamid64: str = None, url_or_username: str = None):
        server_data = self.get_server_data(ctx.guild.id)
        if not server_data:
            await ctx.respond(CONFIG_NOT_INITIALIZED_MESSAGE, ephemeral=True)
            return
        ...
        steamid64 = self.get_steam_id(ctx, steamid64, url_or_username)
        ...
        try:
            steam_id = self.resolve_steam_id(steamid64=steamid64, url_or_username=url_or_username)
            if not steam_id:
            ...
            bans = self.get_player_bans(steam_id) or {}
            player_level = self.get_player_level(steam_id)
        ...
        if is_private_profile:
            ...
            try:
                time_created = self.get_profile_creation_timestamp(summary, steam_id)
            except requests.RequestException:
                time_created = summary.get('timecreated')
        ...
        try:
            owner_id = self.services.firebase.search_record_id(str(ctx.guild.id), 'Users', 'steam', str(steam_id))
    ...
    @discord.option('steamid64', description='SteamID64', required=False)
    @discord.option('url_or_username', description='Ссылка на профиль или имя пользователя после ../id/', required=False)
    async def connect(self, ctx: discord.ApplicationContext, steamid64: str = None, url_or_username: str = None):
        server_data = self.get_server_data(ctx.guild.id)
        if not server_data:
            await ctx.respond(CONFIG_NOT_INITIALIZED_MESSAGE, ephemeral=True)
            return
        ...
        self.services.firebase.update_record(str(ctx.guild.id), 'Users', author_id, {'steam': str(steam_id)})
        embed = discord.Embed(
